...
% Last Modified by GUIDE v2.5 12-Jan-2018 13:33:36
...
istep = 1;
cprintf('\n');
cprintf('*blue','Scanbox v4.0 | https://scanbox.org/ | Dario Ringach (darioringach@me.com)\n\n');
pause(1);
...
retCode = ...
    ...
    boardHandle,		...	% HANDLE -- board handle
    uint32(AC_COUPLING),		...	% U32 -- external trigger coupling id
    uint32(ETR_5V)				...	% U32 -- external trigger range id
    );
if retCode ~= ApiSuccess
...
if(sbconfig.tri_knob_ver>1)
    panel_list = { {handles.realtime} {handles.knobby_sched} {handles.sspanel} {handles.message_panel}  {handles.notes} {handles.optogenetics_panel} {handles.intrinsic_panel} };
else
    panel_list = { {handles.realtime} {handles.message_panel}  {handles.notes} {handles.optogenetics_panel} {handles.intrinsic_panel}};
end
...
ttltimer = timer('ExecutionMode','FixedRate','Period',.1,'TimerFcn',@sb_callback,'Tag','TTL_Timer');
...
size_T_1 = 0; size_T = 0; % store trigger events from MWorks, # of triggers
temp_frame = [];
%%%
...
%         ttlflagnew = bitget(bufferOut.Value(1),2,'uint16');
        ...
        sb_callback; % call fread real time for trigger
        if ~isempty(T)
            temp_frame = T(:,1)+T(:,2)*256; % store frames of trigger
            if temp_frame(1) == 0 % in case the first value is 0
                temp_frame(1) = [];
            end
        end
            
        if ~isempty(temp_frame) % if a real trigger happens, store the frame
            size_T = size(T(:,1)+T(:,2)*256,1);
        end
        
        % updating ttlflagnew using Scanbox instead of Alazartech
        if size_T ~= size_T_1 && ttlflagnew == 0
            ttlflagnew = uint16(1); 
        elseif size_T ~= size_T_1 && ttlflagnew == 1
            ttlflagnew = uint16(0); 
        end
        ...
        if(mm_flag && ((fid ~= -1) || handles.onfocus.Value))     % if grabbing or onfocus flag is on
            if(mmfile.Data.header(1)<0)                           % data was consumed?  If not, move on...  Server loses a frame
                mmfile.Data.header(4) = ttlflagnew;               % ttl flag 
                mmfile.Data.chA = chA';
                mmfile.Data.chB = chB';
        ...
        if(nroi>0)
            ...
            if(stim_flag)
                ...
                if(ttlflag == 0 && ttlflagnew ~= 0) % new stim
                    %%%
                    fprintf(sbudp, 'L1'); % turn laser on 
                    %%%
                ...
                elseif (ttlflag ~= 0 && ttlflagnew == 0) % end stim
                    %%%
                    fprintf(sbudp, 'L0'); % turn laser off
                    %%%
...
istep = 1;
cprintf('\n');
cprintf('*blue','Scanbox Yeti (v4.0) - by Dario Ringach (darioringach@me.com)\n');
...
% 
% try
%     delete(20); % delete fig if existing...
%     slmfig = [];
% catch
% end
% 
...
% figure(20);
% colormap(gray(256));
% slmimg = image('CData',uint8(zeros(size(xh))));
% axis off;
% figure(scanbox_h);

% load calibration
% slm = load(sbconfig.slmcal,'xhat','yhat','tform','rwidth','rheight','x0','y0','I','pts'); % load calibration file
...
for(j=1:length(cellpoly))
    ...
    cellgain(j) = slm.power_interp(aa,bb);                           % equalize power
    if(isnan(cellgain(j))) 
        cellgain(j) = 1;
    end
...
if s(2) == 'n'
    ...
    if(n>0)
        ...
        stop(ttltimer); % stop the timer... called manually in loop
...
% --- Executes during object creation, after setting all properties.
function stimmark_CreateFcn(hObject, eventdata, handles)
% hObject    handle to stimmark (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
