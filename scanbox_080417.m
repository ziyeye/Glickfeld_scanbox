...
% Last Modified by GUIDE v2.5 02-Aug-2017 07:22:56
...
if sbconfig.stimmark == 0
    set(handles.stimmark,'Enable','off');
end
...
if(sbconfig.tri_knob_ver>1)
    panel_list = { {handles.realtime} {handles.knobby_sched} {handles.message_panel}  {handles.notes} {handles.optogenetics_panel} };
else
    panel_list = { {handles.realtime} {handles.message_panel}  {handles.notes} };
end
panel_sel = 1;
set_panel_visible;
...
if(sbconfig.slm)
    ...
    slm = load(sbconfig.slmcal,'xhat','yhat','tform','rwidth','rheight','x0','y0','I','pts','calpts','power_interp'); % load calibration file
...
% moved to blist selection
% idx = str2double(handles.blist.String{handles.blist.Value});
% slmimg.CData = cellphase{idx};
% drawnow;
% pause(0.035);   % needed to update the holoeye ?
...
switch sel
    case 'No'
        return;
    case 'Yes'
        spatialcalibration;  %% do a spatial calibration
end
...
% bring exposure down
handles.dalsa_exposure.Value = 0.05;
handles.dalsa_exposure.Callback(handles.dalsa_exposure,handles);
...
while(true)
    ...
    handles.slmpower.Value = min(sbconfig.slm_validation_power / factor,5);
    handles.slmpower.Callback(handles.slmpower,handles);
...
uisave({'tform','cal','pts','rectx','recty','x0','y0','xhat','yhat','rwidth','rheight','I','img','calpts','mask','slm_etl','power_interp'});
...
global slm
slm = load(sbconfig.slmcal,'xhat','yhat','tform','rwidth','rheight','x0','y0','I','pts','calpts','slm_etl','power_interp'); % load calibration file
...
p = sum(img0_h.CData(:)>sbconfig.agc_threshold)/prod(size(img0_h.CData));
